...
const TYPE_CONFIG: Record<string, { icon: React.ReactNode; color: string; bg: string }> = {
  ...
  Email: { icon: <Mail className="h-4 w-4" />, color: "text-green-600", bg: "bg-green-50" },
  Meeting: { icon: <Calendar className="h-4 w-4" />, color: "text-purple-600", bg: "bg-purple-50" },
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{activities.length} total activities logged</p>
        </div>
        <button
      ...
      {grouped.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 flex flex-col items-center">
          ...
          <p className="text-sm font-medium text-slate-900">No activities found</p>
          <p className="text-xs text-slate-500 mt-1">Log your first activity to get started.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-6">
          {grouped.map(([dateKey, items]) => (
            <div key={dateKey}>
              ...
              <div className="flex flex-col gap-2 ml-4 border-l-2 border-slate-200 pl-4">
                {items.map((a) => {
                  const config = TYPE_CONFIG[a.type] || TYPE_CONFIG.Note;
                  return (
                    <div
                      key={a.id}
                      className="bg-white rounded-lg border border-slate-200 p-4 hover:shadow-sm transition-shadow relative"
                    >
                      <div className="flex items-start gap-3">
                        <div className={`h-9 w-9 rounded-lg ${config.bg} ${config.color} flex items-center justify-center shrink-0 -ml-8`}>
                          {config.icon}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className={`text-xs font-medium ${config.color} ${config.bg} px-2 py-0.5 rounded`}>{a.type}</span>
                        ...
                        <button
                          ...
                          className="p-1 rounded-md hover:bg-red-50 text-slate-300 hover:text-red-500 transition-colors shrink-0"
                          title="Delete"
                        >
